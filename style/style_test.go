...
func testText() (s string) {
	...
	s += P("Notably, package '%s' can print in %s or %s (if schosen style supports it).", Underline("style"), Red("red"), Bold(Green("bold green")))
	...
	s += L("It is also possible to check that line inside lists are respected.\nAs you can see here in this simple example.")
