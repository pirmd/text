...
func testText() (s string) {
	...
	s += L("It is also a nice-to have feature that sub-lists are supported")
	s += Tab(L("Sub-list is here"))
	s += Tab(L("Sub-list is also here"))
	s += L("It is also possible to have a list item containing several paragraphs")
